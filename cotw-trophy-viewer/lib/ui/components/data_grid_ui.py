        ...
        self.dataGrid = ui.aggrid({
            ...
            'columnDefs': [
                ...
                {'headerName': 'Harvested Date', 'field': 'datetime', 'sort': 'desc',
                 'valueFormatter': self.TIME_FORMAT, 'width': '150', 'filter': 'agDateColumnFilter'},
                {'headerName': 'Slot', 'field': 'slotId', 'width': '100'},
                {'headerName': 'Multi Mount', 'field': 'isHybrid', 'width': '100'},
            ],
            'pagination': True,
...
def rowData(trophyAnimals: List[TrophyAnimal]) -> List[dict]:
    ...
    for animal in trophyAnimals:
        rows.append({
            ...
            'isHybrid': _naIfNone(animal.isHybrid, lambda h: 'Yes' if h else 'No'),
